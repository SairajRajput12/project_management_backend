...
def f_update_project():
    print('Updating project starting!')
    ...
    # Validate required fields in project_data
    required_fields = ['name', 'starting_date', 'ending_date', 'status', 'project_manager', 'project_manager_email', 'id']
    ...
    try:
        ...
        if connection.is_connected():
            ...
            query = """
                UPDATE projects 
                SET name = %s, starting_date = %s, ending_date = %s, status = %s, 
                    project_manager = %s, project_manager_email = %s 
                WHERE id = %s
            """
            cursor.execute(query, (
                project_data['name'], 
                project_data['starting_date'], 
                project_data['ending_date'], 
                project_data['status'], 
...
def df_update_project():
    ...
    try:
        ...
        if connection.is_connected():
            ...
            query1 = "Delete from projects where id=%s;"
            cursor.execute(query1, (project_id))
            connection.commit()  
...
def signup():
    ...
    try:
        ...
        if connection.is_connected():
            ...
            if last_row:
                prev_id = last_row['id'] + 1
            else:
                prev_id = 101

            query2 = "INSERT INTO users (id, username, email, password,role) VALUES (%s, %s, %s, %s,%s);"
            cursor.execute(query2, (prev_id, username, email, password,role))
            connection.commit()  
...
def n_update_tasks(): 
    ...
    if not project_user_id or not task_id or not task_data or not task_status:
    ...
    try:
        ...
        if connection.is_connected():
            ...
            query = """
                ...
                WHERE id = %s AND project_user_id = %s
            """
            cursor.execute(query, (task_data, task_status, task_id, project_user_id))
...
def add_project():
    ...
    if not project_data:
        return jsonify({"message": "Project data is not supplied!"}), 400

    # Extract and validate required fields
    required_fields = ['name', 'description', 'Starting_Date', 'Ending_Date', 'ProjectStatus', 
                       'Project_Manager', 'Project_manager_mail', 'admin', 'Users']
    ...
    try:
        ...
        if connection.is_connected():
            ...
            cursor.execute(query1)
            last_row = cursor.fetchone()
            prev_id = last_row['id'] + 1 if last_row else 1001

            # Insert into the `projects` table
            query2 = """
                INSERT INTO projects 
                (id, name, description, starting_date, ending_date, status, project_manager, 
                 project_manager_email, admin_id) 
            ...
            cursor.execute(query2, (
                ...
                project_data['Starting_Date'], 
                project_data['Ending_Date'], 
                project_data['ProjectStatus'], 
                project_data['Project_Manager'], 
                project_data['Project_manager_mail'], 
                project_data['admin']
            ))

            # Get the last project_user ID
            query3 = "SELECT id FROM project_users ORDER BY id DESC LIMIT 1;"
            cursor.execute(query3)
            last_row = cursor.fetchone()
            prev_project_id = last_row['id'] + 1 if last_row else 1

            # Get the last task ID
            query4 = "SELECT id FROM tasks ORDER BY id DESC LIMIT 1;"
            cursor.execute(query4)
            last_row = cursor.fetchone()
            prev_task_id = last_row['id'] + 1 if last_row else 1

            # Insert project users and tasks
            for user in project_data['Users']:
                project_user_id = user.get('user_id')
                user_progress = user.get('progress', 0)
                user_role = user.get('role', 'Member')

                # Insert into `project_users` table
                query5 = """
                    INSERT INTO project_users 
                    (id, project_id, user_id, progress, role) 
                    VALUES (%s, %s, %s, %s, %s);
                """
                ...
                for task in user_tasks:
                    ...
                    query6 = """
                        INSERT INTO tasks 
                        (id, project_user_id, task_name, task_status) 
                        VALUES (%s, %s, %s, %s);
                    """
            ...
            connection.commit()
            return jsonify({"message": "Project and related data added successfully!"}), 200
...
def admin_signup():
    ...
    try:
        ...
        if connection.is_connected():
            ...
            # Fetch the last ID
            query1 = "SELECT id FROM admin ORDER BY id DESC LIMIT 1;"
            cursor.execute(query1)
            last_row = cursor.fetchone()
            ...
            query2 = "INSERT INTO admin (id, username, email, password) VALUES (%s, %s, %s, %s);"
            cursor.execute(query2, (prev_id, username, email, password))
            connection.commit()  
...
def login():
    ...
    try:
        ...
        if connection.is_connected():
            ...
            return jsonify({"message": "Invalid username or password.please signup first"}), 401
...
@app.route('/fetch_admin_project', methods=['POST'])
def get_projects():
    print('Fetching admin projects...')
    data = request.json
    admin_id = data.get('admin_id')  # Getting admin_id from the request body
    ...
    try:
        ...
        if connection.is_connected():
            ...
            # Correct the placeholder format for MySQL
            query1 = "SELECT * FROM projects WHERE admin_id = %s"
            cursor.execute(query1, (admin_id,))  # Use a tuple for the parameter
            admin_project_data = cursor.fetchall()

            return jsonify({
                "message": "Admin project data fetched successfully!",
                "admin_data": admin_project_data
            }), 200
...
@app.route('/fetch_project_user_task_data', methods=['POST'])
def give_admin_project_user_task_data():
    print('Fetching admin projects...')
    data = request.json
    project_id = data.get('project_id')  # Getting admin_id from the request body
    ...
    try:
        ...
        if connection.is_connected():
            ...
            # Correct the placeholder format for MySQL
            query1 = "SELECT * FROM project_users WHERE project_id = %s"
            cursor.execute(query1, (project_id,))
            admin_project_users = cursor.fetchall()
            
            for user in admin_project_users: 
                user_project_id = user['id'] 
                query2 = "select * from tasks where project_user_id=%s" 
                cursor.execute(query2,(user_project_id,)) 
                user_task_data = cursor.fetchall() 
                user['tasks'] = user_task_data 
            return jsonify({
                "message": "Admin project data fetched successfully!",
                "admin_data": admin_project_users
            }), 200
...
def fetch_manager_projects():
    ...
    data = request.json
    print(data)
    manager_name = data.get('manager_name')  # Getting user_name from the request body
    ...
    try:
        ...
        if connection.is_connected():
            ...
            # Correct the placeholder format for MySQL
            query1 = "SELECT * FROM projects WHERE project_manager = %s and project_manager_email=%s;"
            cursor.execute(query1, (manager_name,manager_email))  # Use a tuple for the parameter
            admin_project_data = cursor.fetchall()

            return jsonify({
                "message": "manager project data fetched successfully!",
                "manager_data": admin_project_data
            }), 200
...
@app.route('/fetch_tasks_for_manager', methods=['POST'])
def get_user_tasks():
    print('Fetching user projects...')
    data = request.json
    print(data)
    ...
    try:
        ...
        if connection.is_connected():
            ...
            return jsonify({
                "message": "manager project data fetched successfully!",
                "manager_data": admin_project_users
            }), 200
